...
Suite à l'introdutcion d'un individu infecté par l'agent pathogène dans la population, une vague de propagation se déclanche. Elle est rapidement stabilisée car de nombreux individus sont immunisés. Après 2 ans les proportions d'individus au sein de chaque classes sont stabilisés, les sorties calculées le dernier jour sont donc stables.\
...
# Matrice des paramètres nominaux
...
param_matrix_var <- matrix(0, 
                           nrow = 10, 
                           ncol = length(ValNominale))
#Remplir la matrice des paramètres variants
for (i in 1:length(ValNominale)) {
  param_matrix_var[, i] <- seq(ParamRanges$lower[i],
                               ParamRanges$upper[i], 
                               length.out = 10)
  }

# Créer un dataframe qui stock les résultats de l'analyse de sensibilité
SensitivityResults <- data.frame(Parameter = character(0), 
...
abline(a = 0, b = 1); abline(a = 0, b = 0.5, lty = 2); abline(a = 0, b = 0.1, lty = 3)
...
-   Le taux d'infectés le dernier jours de la modélisation est influencé de manière monotone par les paramètres 11, 12, 13 et 14. Ce sont donc les paramètres **trans**, **lat**, **rec** et **loss**. Les variations de ces paramètres sont constantes. une sensibilité des paramètres **loss** et **rec** a déjà été mise en avant par la méthode OAT.\
-   L'incidence du dernier jours est modifiée de par le paramètre **rec** toujours de manière monotone. Elle est également influencée de manière monotone par les paramètres **K**, **sr**, **m3**, **f3**, **portee**, **trans** et **loss**. Les paramètres **K**, **trans** et **loss** ont déjà été mis en avant par la méthode OAT.\
-   Le nombre maximum d'infectieux est influencée par les paramètres **sr**, **f3**, **portee** toujours de manière monotone. Les paramètres **K**, **trans** et **lat** influencent de manière monotone le nombre maxium d'infectieux. Seuls les paramètres **K** et **trans** ont été identifiés par la méthode OAT.\
-   L'incidence sur l'année est influencée par les paramètres **sr**, **f3**, **portee** et **rec** toujours de manière monotone ainsi que par les paramètres **K**, **trans** et **loss** de manière monotone. La méthode OAT avait mis en évidence la sensibilité des 3 paramètres monotones.\

La sensibilité des paramètres monotones sont détectés par l'analyse OAT mais pas les autres classes de sensibilité.
...
Ce package permet d'utiliser des méthodes d'analyse de sensibilité globales en plus du screening comme précédemment avec les méthodes OAT et Morris. C'est-à-dire que l'on peut prendre en compte le caractère continu des variables d'entrée et les intéractions entre elles. On peut donc étudier les effets **principaux** (paramètre seul) et **totaux** (intéractions possibles avec d'autres paramètres + paramètre seul) des variables d'entrée. Ce sont des méthodes basées sur la décomposition de la variance. Dans cette catégorie, nous pouvons retrouver la méthode **FAST**, ou 'Fourier Amplitude Sensitivity Test', mais encore la méthode du plan factoriel ou Sobol.

Le principe est de choisir pour cette étude : - Les variables d'entrée que l'on souhaite suivre. *Dans notre cas, nous choisissons tous les paramètres d'entrée.* - Leur plage de variation, avec une valeur minimale et une valeur maximale qui bornent cette plage. *Nous choisissons une variation de +/- 25% de la valeur de base du paramètre afin de rester dans des valeurs réalistes pour le modèle.* - Une loi de distribution qui régit la plage de variation. *Nous avons choisi une loi uniforme ('qunif') afin d'avoir une variation homogène de nos paramètres et de n'avoir aucun a priori sur ces derniers.*
...
Maintenant qu'on a le design experimental, on peut regarder comment les variations des paramètres remplissent l'espace. Ici, on représente les variations des paramètres 2 à 2 ici on ne représente que les paramètres K et sr.
...
*A priori*, on a l'impression que pour 100 scénarios, on a un recouvrement plus homogène de l'espace qui suggèrerait potentiellement une analyse FAST plus précise et plus fiable. En revanche, si l'on compare les deux recouvrements, on se rend compte que pour 100 simulations, il y a plus de trous à certains endroits, ce qui rend donc ce paramétrage moins efficace que pour 1000 scénarios.
...
Cette sortie est sensible à deux paramètres qui sont trans et K en effet principal. La présence de trans s'explique par l'équation ci-dessus, et K par le fait que c'est un facteur limitant de S, I et N.

**Sortie 3 : Nombre maximum d'infections sur 2 ans**

$$ \text{Nombre maximum d'infectieux en 2 ans} = max(I_{N_t})$$

Le nombre maximum d'infectieux en 2 ans est surtout sensible à K, trans, lat et rec.

Pour rappel, l'équation permettant d'obtenir $I_{N_t}$ est : $$I_{N_{t}}=I_{NN_{t}}+I_{J_{t}}+I_{A_{t}}$$

Avec : $$I_{NN{t+1}} = I_{NN_t}(1 - m_{1} - madd - t_{1} - rec) + lat . L_{NN_t}$$ $$I_{J_{t+1}} = I_{NN_t}.t_{1} + I_{J_t}(1 - m_{2} - madd - t_{2} - rec) + lat.L_{J_t}$$ $$I_{A_{t+1}}= I_{J_t}.t_{2} + I{A_t}(1 - m_{3} - madd - rec) + lat.L_{A_t}$$

Où l'on retrouve tous ces paramètres (K apparaît intrinsèquement au fait qu'il a une influence sur le nombre d'individus apparaissant dans la population dû aux naissances par les adultes). La mort dûe à l'AP ne semble pas avoir d'incidence majeure sur ce résultat en revanche.

**Sortie 4 : Incidence de l'AP sur 1 an**

$$ \text{Incidence de l'AP sur 1 an} = \sum_{t=1}^{365}\frac{transS_{N_t}I_{N_t}}{N_t}$$ Pour ce résultat, ce sont K, trans et loss pour lequel il est le plus sensible. Trans est exprimé explicitement dans la formule ci-dessus et K, comme pour la sortie 2. Loss est important ici contrairement à la sortie 2 car on étudie une période de 2 ans et non un maximum.
