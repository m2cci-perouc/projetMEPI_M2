...
Suite à l'introdutcion d'un individu infecté par l'agent pathogène dans le modèle, une vague de propagation se déclanche. Elle est rapidement stabilisée car de nombreux individus sont immunisés. Après 2 ans les proportions d'individus dans chaque classes sont stabilisés, les sorties qui sont calculées le dernier jour sont donc les sorties stables.\
...
Visualiser les résultats de l'analyse Morris.

```{r}
par(mfrow = c(1, 2))
plot(morris_results, y_col = 1, main = "Taux d'infectés le dernier jours")
...
Lien pour la méthode sur R : <https://rdrr.io/cran/fast/man/sensitivity.html> \### a. Le package utilisé pour l'analyse de sensibilité avec la méthode FAST est 'sensitivity'.
...
Maintenant qu'on a le design exprimental, on peut regarder comment les variations des paramètres remplissent l'espace. Ici, on représente les variations des paramètres 2 à 2 ici on ne représente que les paramètres K et sr.
...
Nous faisons l'hypothèse que la transmission de l'agent pathogène est plus grande au sein d'une même classe qu'entre les classes d'âge car les individus de même classe d'âge se retrouve plus souvent proches.
...
Afficher les graphiques de l'analyse de Morris sur le modèle modifié.
...
**Design expérimental** On fait donc l'analyse de sensibilité FAST pour 1000 simulations.
