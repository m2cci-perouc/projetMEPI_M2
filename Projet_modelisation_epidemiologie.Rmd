...
-   La sortie 1 qui exprime le taux d'infectés le dernier jours de la modélisation est modifiée de manière monotone par les paramètres 11, 12, 13 et 14. Ce sont donc les paramètres **trans**, **lat**, **rec** et **loss**. Les variations de ces paramètres sont donc constantes. Les paramètres **loss** et **rec** étaient déjà mis en avant par la méthode OAT.\
-   La sortie 2 qui mesure l'incidence du dernier jours est toujours modifiée de manière monotone par le paramètre 13 soit **rec**. Elle est également influencée de manière monotone par les paramètres 1, 2, 5, 7, 8, 11 et 14 soit **K**, **sr**, **m3**, **f3**, **portee**, **trans** et **loss**. Les paramètres **K**, **trans** et **loss** ont été mis en avant par la méthode OAT.\
-   La sortie 3 est influencée par les paramètres 2, 7 et 8 toujours de manière monotone (**sr**, **f3**, **portee**). Les paramètres 1, 11 et 12 soit **K**, **trans** et **lat** influencent de manière monotone le nombre maxium d'infectieux. Seuls les paramètres **K** et **trans** ont été identifiés par la méthode OAT.\
-   La sortie 4 est influencée par les paramètres **sr**, **f3**, **portee** et **rec** toujours de manière monotone ainsi que par les paramètres **K**, **trans** et **loss** de manière monotone. La méthode OAT avait mis en évidence la sensibilité des 3 paramètres monotones.\

Les paramètres influeçants le plus les sorties sont **K**, **sr**, \*\*portee\*, **trans** et **loss**. Nous ne retrouvons pas le paramètre **rec** qui étaient sensible dans l'analyse OAT.
...
# On fait 4 fois le FAST pour séparer les différentes sorties
si<-1
FAST1A<- fast99(model = modApplimodif2, factors = ParamNames, n = 1000, q = rep("qunif",16), q.arg = ParamRanges)
si<-2
FAST1B<- fast99(model = modApplimodif2, factors = ParamNames, n = 1000, q = rep("qunif",16), q.arg = ParamRanges)
si<-3
FAST1C<- fast99(model = modApplimodif2, factors = ParamNames, n = 1000, q = rep("qunif",16), q.arg = ParamRanges)
si<-4
FAST1D<- fast99(model = modApplimodif2, factors = ParamNames, n = 1000, q = rep("qunif",16), q.arg = ParamRanges)
```
