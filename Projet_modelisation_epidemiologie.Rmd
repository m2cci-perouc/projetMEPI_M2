...
modAppli <- function(parametre){  
  ...
  for (i in 1:nrow(parametre)) { 
    ...
    #Plot
    plot(x = NA, y = NA, xlim = c(0, 730), ylim = c(0, 35),
...
des_xp<-des_xp100$X # On conserve le tableau avec les variations par paramètres
length(des_xp[,1]) # Longueur du tableau
...
```{r}
plot(des_xp[,1:2], type='p', col = c("red", "blue"), main = "Variation des paramètres K et sr dans l'espace des paramètres") # On regarde comment les paramètres remplissent l'espace
legend("topright", legend = c("K","sr"), text.col = c("red", "blue"))
```
