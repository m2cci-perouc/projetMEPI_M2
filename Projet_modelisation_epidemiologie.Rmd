...
### a. Informations sur les modifications du modèle

Nous faisons l'hypothèse que la transmission de l'agent pathogène est plus grande au sein d'une même classe qu'entre les classes d'âge. Nous pouvons imaginer cette situation dans un élevage avec des individus de la même classe d'âge qui sont parqués au même endroit
...
modApplimodif <- function(parametre){  
  ...
  for (i in 1:nrow(parametre)) { 
    ...
    f2 = parametre[i,6];	
    f3 = parametre[i,7];	
    portee = parametre[i,8];	
    t1 = parametre[i,9];	
    t2 = parametre[i,10];	
    
  
    trans_inter = parametre[i,11]; 
    trans_intra = parametre[i,12] 
    lat = parametre[i,13];	
    rec = parametre[i,14];	
    loss = parametre[i,15];	
    madd = parametre[i,16];	
