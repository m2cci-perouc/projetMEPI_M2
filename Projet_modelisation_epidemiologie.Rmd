...
La variation des paramètres **K**, **loss**, **rec** et **trans** ont un impact sur les sorties du modèle, ils sont donc sensibles aux valeurs d'entrée.\
...
- Le paramètre **loss** soit le taux de perte de l'immunité d'un individu impact également la dynamique de la maladie. Quand ce taux augmente, le taux d'infectés le dernier jours et les incidences augmentent également.\
- Le paramètre **rec** (taux d'immunisation) diminue légèrement le taux d'infecté le dernier jour ainsi que le nombre d'infectieux total.\
- **sr** exprime le sex-ratio.\
- Le paramètre **trans** (taux de transmission) augmente le taux d'infectés, le nombre d'infectieux et les incidences quand il augmente.\
Globalement, les paramètres avec les plus grandes sensibilités sont **K**, **loss** et **rec**.
...
Les bornes ont été réalisées en s'appuyant sur l'article de (Menberg, 2016).\
Menberg, Kathrin & Heo, Y & Choudhary, R. (2016). Sensitivity analysis methods for building energy models: Comparing computational costs and extractable information. Energy and Buildings. 133. 433-445. 10.17863/CAM.6789.\
Ces bornes permettent de délimiter les paramètres dans des classes différentes.\
...
-   Le taux d'infectés le dernier jours est modifiée de manière monotone par les paramètres 11, 12, 13 et 14. Ce sont donc les paramètres **trans**, **lat**, **rec** et **loss**. Les variations de ces paramètres sont constantes. Les paramètres **loss** et **rec** ont déjà été mis en avant par la méthode OAT.\
-   L'incidence du dernier jours est toujours modifiée de manière monotone par le paramètre **rec**. Elle est également influencée de manière monotone par les paramètres **K**, **sr**, **m3**, **f3**, **portee**, **trans** et **loss**. Les paramètres **K**, **trans** et **loss** avait été mis en avant par la méthode OAT.\
-   Le nombre d'infectieux est influencée par les paramètres **sr**, **f3**, **portee** qui sont toujours monotone. Les paramètres **K**, **trans** et **lat** influencent de manière monotone le nombre maxium d'infectieux. Seuls les paramètres **K** et **trans** ont été identifiés par la méthode OAT.\
-   L'incidence à l'année est influencée par les paramètres **sr**, **f3**, **portee** et **rec** toujours de manière monotone ainsi que par les paramètres **K**, **trans** et **loss** de manière monotone. La méthode OAT avait mis en évidence la sensibilité des 3 paramètres monotones.\

Les paramètres influeçants le plus les sorties sont **K**, **sr**, **portee**, **trans** et **loss**.\
La méthode OAT ne détérmine la sensibilité que des paramètres de classe monotone.
...
des_xp100<-fast99(model = NULL, 
                  factors = ParamNames, 
                  n = 100, 
                  q = rep("qunif",15), 
                  q.arg = ParamRanges) # Pour creer le design expérimental
...
Maintenant qu'on a le design experimental, on peut regarder comment les variations des paramètres remplissent l'espace.
...
```{r, echo = F}
par(mfrow=c(2,2))
hist(FAST2$y[1:15000], main = "Analyse de sensibilité FAST : sortie 1", xlab = "Valeurs de la sortie 1")
hist(FAST2$y[15001:30000], main = "Analyse de sensibilité FAST : sortie 2", xlab = "Valeurs de la sortie 2")
hist(FAST2$y[30001:45000], main = "Analyse de sensibilité FAST : sortie 3", xlab = "Valeurs de la sortie 3")
hist(FAST2$y[45001:60000], main = "Analyse de sensibilité FAST : sortie 4", xlab = "Valeurs de la sortie 4")
```
...
si<-1
FAST1A<- fast99(model = modAppli2, 
                factors = ParamNames, 
                n = 100, 
                q = rep("qunif",15), 
                q.arg = ParamRanges)
si<-2
FAST1B<- fast99(model = modAppli2, 
                factors = ParamNames, 
                n = 100, 
                q = rep("qunif",15), 
                q.arg = ParamRanges)
si<-3
FAST1C<- fast99(model = modAppli2, 
                factors = ParamNames, 
                n = 100, 
                q = rep("qunif",15), 
                q.arg = ParamRanges)
si<-4
FAST1D<- fast99(model = modAppli2, 
                factors = ParamNames, 
                n = 100, 
                q = rep("qunif",15), 
                q.arg = ParamRanges)
...
```
Pour chaque sortie, on obtient les indices de sensibilité pour chaque paramètre, en prenant en compte les effets principaux et les intéractions. 

**Sortie 1 : Taux d'infectés (latents et infectieux) le dernier jour : ** 

Pour cette sortie, on observe une sensibilité particulière pour les paramètres K, sr, f3 et portee (en intéraction) et m1, m3, t2, trans, lat, rec et loss pour des effets principaux, m2 en moindre mesure. 

$$ \text{Taux d'infectés (latents et infectieux) le dernier jour} = \frac{L_{N_t} + {I_{N_t}}}{N_t}$$\

D'après l'équation, nous pouvons voir que ce résultat dépend de $L_{N_t}$, $I_{N_t}$ et ${N_t}$.

Ces variables dépendent elles-même de quasiment tous les paramètres, hormis ce qui concerne les individus sains et remis, soit les paramètres f2, t1 et madd. 

Les paramètres K et f3 sont en intéraction pour les naissances dans le compartiment des sains par les adultes. Nous pouvons donc imaginer qu'une population en croissance, avec un effectif grandissant, peut influencer fortement le nombre d'infectés par rapport à la population totale après un épisode d'épidémie. Ce constat peut être logique si l'on pose que plus d'individus dans une population peut induire plus de cas d'infection. Nous pouvons imaginer la même chose pour sr et portée dans le cas de la reproduction des jeunes, mais de manière légèrement moins marquée (les jeunes se reproduisant moins que les adultes).

Pour la mortalité naturelle (m1, m2 et m3), ces paramètres influent de manière importante sur la sortie, au même titre que les naissances. Nous pouvons remarquer que le taux d'infectés le dernier jour est moins sensible à m2, qui cible la classe des jeunes, qui ont une mortalité moins importante que les 2 autres classes. Le passage de la classe des jeunes à adultes constitue une sortie vers une classe qui se reproduit plus (les adultes), et donc va avoir une influence sur le résultat.

Les paramètres liés au pathogène (trans, lat, rec, loss) vont naturellement influencer le résultat car il concerne le taux d'infection final. 

**Sortie 2 : Incidence de l'AP le dernier jour**

$$ \text{Incidence de l'AP le dernier jour} = \frac{transS_{N_t}I_{N_t}}{N_t}$$\

Cette sortie est sensible à deux paramètres qui sont loss et K en effet principal. 
La présence de loss s'expliquerait par le fait que la perte d'immunité ajoute des individus dans le compartiment S (et ce de mannière plus efficace que par les naissances). Pour K, par le fait que c'est un facteur limitant des effectifs des classes d'âge, ce qui est logique car plus les effectifs dans la population d'individus sains est importante, plus d'individus peuvent être contaminés et contaminer en retour. 

**Sortie 3 : Nombre maximum d'infections sur 2 ans**

$$ \text{Nombre maximum d'infectieux en 2 ans} = max(I_{N_t})$$\

Le nombre maximum d'infectieux en 2 ans est surtout sensible à K, trans, lat et rec en effet principal, soit des paramètres concernant la limite des effectifs dans la population et relatifs à l'agent pathogène. 

Pour rappel, l'équation permettant d'obtenir $I_{N_t}$ est : 
$$I_{N_{t}}=I_{NN_{t}}+I_{J_{t}}+I_{A_{t}}$$
Avec : 
$$I_{NN{t+1}} = I_{NN_t}(1 - m_{1} - madd - t_{1} - rec) + lat . L_{NN_t}$$\
$$I_{J_{t+1}} = I_{NN_t}.t_{1} + I_{J_t}(1 - m_{2} - madd - t_{2} - rec) + lat.L_{J_t}$$\
$$I_{A_{t+1}}= I_{J_t}.t_{2} + I{A_t}(1 - m_{3} - madd - rec) + lat.L_{A_t}$$
Où l'on retrouve tous ces paramètres (K apparaît intrinsèquement du fait qu'il a une influence sur le nombre d'individus apparaissant dans la population dû aux naissances par les adultes). Les pertes, et donc la mort dûe à l'AP ne semble pas avoir d'incidence majeure sur ce résultat en revanche puisque l'on recherche une valeur maximale. 

**Sortie 4 : Incidence de l'AP sur 1 an**

$$ \text{Incidence de l'AP sur 1 an} = \sum_{t=1}^{365}\frac{transS_{N_t}I_{N_t}}{N_t}$$
Pour ce résultat, ce sont K, trans et loss en effets principaux pour lequel il est le plus sensible. Trans est exprimé explicitement dans la formule ci-dessus et K, comme pour la sortie 2. Loss est important ici contrairement à la sortie 2 car on étudie une période de 2 ans et non un maximum. Ce paramètre est important pour l'entrée de nouveaux individus dans le compartiment 'S'.

## 5. Discussion

Entre les méthodes d'analyse de sensibilité de OAT et Morris, nous ne décelons peu de différences entre les résultats pour chaque sortie. Néanmoins, la méthode OAT détécte seulement les paramètres qui ont une sensibilité monotone. Dès que le paramètre a une sensibilité en interaction avec une autre elle ne sera pas détectée. Entre ces deux méthodes, les résultats sont donc cohérants. La méthode OAT est très rapide mais ne permet pas une analyse très poussée. De plus, la méthode Morris ne prend pas beaucoup plus de temps à tourner.

Concernant FAST, comme Morris, l'espace des paramètres est plus exploré que pour OAT. En revanche, ce recouvrement des possibilités n'est pas représenté par des grilles comme Morris, ce qui le rend plus fin, car explore plus de possibilités de valeurs des paramètres (continus) et les intéractions entre paramètres. Nous pouvons le constater avec les différences marquées obtenues entre 100 et 1000 simulations. Cependant, les temps de simulation sont très coûteux en temps, et peut-être que 1000 simulations sont bien largement assez pour trouver des indices de sensibilité fiables. Il serait donc intéressant de trouver un optimum de nombre de simulations qui permettraient de trouver un compromis entre fiabilité des informations de l'analyse et temps de calcul. 

Pour les résultats, FAST réussit à retrouver les paramètres de type 'monotone' de Morris, mais pas les autres. Cependant, il nous permet de connaître les intéractions, ce qui n'est pas possible avec Morris. 

En conclusion, nous pouvons dire que Morris et FAST sont des méthodes d'analyse de sensibilité complémentaires efficaces pour connaître l'influence des paramètres d'entrée sur les sorties. OAT peut être intéressante dans le cas d'un modèle peu complexe (peu de paramètres et d'intéractions possibles) car elle est assez simple et rapide à utiliser et à interpréter. Dans le cas de la précision portée par FAST et Morris, il serait intéressant de faire des tests qui indiqueraient la significativité des résultats dans le cas du nombre de simulations et d'interprétation des résultats. 
...
```{r, echo = F}
modApplimodifplot <- function(parametre){  
...
En différenciant les transmissions de l'AP entre classe et au sein de la même classe avec pour valeur 0,2 et 0,5. Nous avons mis un taux de transmission important pour la transmission intra-classe car les individus sont très proches les uns des autres. Le modèle n'est pas stabilisé au bout de 2 ans.\
- Le taux d'infectés le dernier jour vaut 0,12.\
- L'incidence au dernier jour de la modélisation vaut 1,00.\
- Le nombre maximum d'infectés en une journée est de 8,2 individus.\
- La somme de l'incidence sur une année vaut 875,6.\
Le taux d'infectés est inférieur au modèle de base et il descend encore. L'incidence au dernier jours est au contraire supérieure. Le nombre maximum d'infectés est nettement inférieur, pourtant la somme de l'incidence sur une année est plus grande.
...
La variation des paramètres **loss**, **rec**, **trans_inter** et **trans_intra** ont un impact sur les sorties du modèle, le modèle est donc sensible à leurs valeurs d'entrée.\
- Le taux de perte d'immunité (**loss**) augmente légèrement le taux d'infectés par jours ainsi que l'incidence à la journée et à l'année quand sa valeur d'entrée est augmentée. Cela veut dire que ce taux a un impact sur moins d'un individu reste longtemps immunisé, plus - Le taux d'immunisation (**rec**) diminue le taux d'infecté par jour, le nombre d'infecté, l'incidence du dernier jour et à l'année en augmentant. Cela veut dire que plus le taux d'immunisation est élevé plus les individus infectés sont immunisés rapidement, la propagation de la maladie est donc diminuée.\
- Le taux de transmission entre individus de la même classe (**trans_inter**) augmente l'incidence du dernier jour et à l'année ainsi que le nombre d'infectieux.\
- Le taux de transmission intra classe (**trans_intra**) influence positivement le taux d'infectés par jours, le nombre total d'infectieux et la somme d'incidence à l'année.\

Dans ce modèle, le paramètre **K** n'a plus une très grande sensibilité. En ravanche, les paramètres **loss**, **rec** et **trans** sont toujours sensibles. Le taux de transmission qui a été détaillé en 2 paramètres : taux de transmission inter et intra classe, le taux **trans_inter** montre une sensibilité supérieure à **trans_intra**.
...
$\sigma$ mesure les interactions et les relations non linéaires entre les paramètres.\
$\mu$* mesure la sensibilité du paramètre.\
Les mêmes bornes que pour l'analyse du modèle de base sont ajoutées.\
L'espace le plus à gauche signifie que le paramètre est non linéaire et/ou non monotone.\
Ensuite, la zone détermine les paramètres toujours monotone.\
Ensuite, les paramètres sont monotones.\
Enfin, les paramètres sont linéaires. \

- Le taux d'infectés le dernier jours est influencé de manière monotone par les paramètres **m1**, **m3**, **t1**, **t2**, **trans_intra**, **lat**, **rec** et **loss**. Ces paramètres sont sensibles de manières monotones, ils ont donc été également identifiés par la méthode OAT.\
- L'incidence le dernier jour est influencée de manière non linéaire et/ou non monotone par le paramètre **t1**. Les paramètres **m1**, **m3**, **t2**, **trans_inter**, **trans_intra**, **rec**, **loss** influence cette sortie de manière monotone. Les paramètres monotones ont tous été identifiés par la méthode OAT.\
- Le nombre maximum d'infectieux en une journée est influencé par les paramètres **m3**, **t2**, **trans_intra**, **lat**, **rec**, ils ont une sensibilité monotone.\
- L'incidence à l'année est influencée par les paramètres **m3**, **t2**, **trans_inter**, **trans_intra**, **lat**, **rec**, **loss** qui ont une sensibilité monotones.
...
```{r, echo = F}
modApplimodif2 <- function(parametre, s = si){  
...
# On fait 4 fois le FAST pour séparer les différentes sorties
si<-1
FAST1A<- fast99(model = modApplimodif2, 
                factors = ParamNames, 
                n = 1000, 
                q = rep("qunif",16), 
                q.arg = ParamRanges)
si<-2
FAST1B<- fast99(model = modApplimodif2, 
                factors = ParamNames, 
                n = 1000, 
                q = rep("qunif",16), 
                q.arg = ParamRanges)
si<-3
FAST1C<- fast99(model = modApplimodif2, 
                factors = ParamNames, 
                n = 1000, 
                q = rep("qunif",16), 
                q.arg = ParamRanges)
si<-4
FAST1D<- fast99(model = modApplimodif2, 
                factors = ParamNames, 
                n = 1000, 
                q = rep("qunif",16), 
                q.arg = ParamRanges)
...
Cette fois, on observe une sensibilité à rec, loss et t2, m3 et trans_intra en moindre mesure pour les effets principaux. 

Dans ce cas, moins de paramètres influent ce résultat et ne sont pas en intéraction. C'est le cas des naissances dans le compartiment des sains par exemple. Ici, le taux d'infectés est influencé par les paramètres concernant le pathogène principalement, ce qui est logique car on s'intéresse au taux d'infectés final. 
...
L'incidence de l'AP le dernier jour est influencée par m3, trans_inter, rec et loss, et t2 et trans_intra en moindre mesure, uniquement en effets principaux.
Dans le premier modèle, cette valeur était sensible à loss et K uniquement. 

Il est possible que trans_inter soit influent car il impacte directement toutes les classes d'âge, tandis que trans_intra est moins influent, car le passage à la classe d'âge supérieur est plus lent. Rec et loss concernent les individus qui entrent et qui sortent d'un état d'immunité, ce qui est important car on souhaite connaître l'effet de l'agent pathogène sur la population, et donc les individus sains. 

**Sortie 3 : Nombre maximum d'infections sur 2 ans**

$$ \text{Nombre maximum d'infectieux en 2 ans} = max(I_{N_t})$$\

Trans_intra et rec en effets principaux sont les paramètres qui vont avoir un effet important sur le nombre maximal d'infectieux sur 2 ans. Nous retrouvons de nouveau rec, qui vient influencer le résultat par la perte de l'état infectieux. En revanche, trans_intra a une inlfuence possiblement positive, car il s'agit d'une valeur élevée de transmission de l'agent pathogène, ce qui va donc avoir plus d'effet que trans_intra par exemple. 
...
Pour ce résultat, ce sont trans_intra, trans_inter, rec et loss en effets principaux pour lesquels il est le plus sensible. On retrouve bien ici trans et loss. Le fait de perdre l'immunité vient contrebalancer les forts taux de transmission. 

**Globalement, l'effet limitant de K n'est plus une composante du modèle qui va avoir une influence importante, contrairement aux deuc facteurs de transmission.** 

<<<<<<< HEAD
### c. Discussion et comparaison entre les modèles

Globalement, les paramètres trans_intra et trans_inter ont une influence plus importante sur le modèle modifié que trans dans le premier modèle. Nous pouvons le constater car les résultats des analyses sont assez différents entre les 2 modèles, en particulier moins d'intéractions sont présentes. 

Les sorties sont devenues plus sensibles aux paramètres liés à l'agent pathogène (en particulier de transmission) que les paramètres démographiques, qui avaient tout de même une certaine influence dans le premier modèle (en particulier K). 

Préciser les transmissions en intra-classe et inter-classe ont donc permis de rendre compte de paramètres influents bien différents dans le cadre de notre exemple, à savoir un élevage avec des classes d'âge parquées ensemble, ce qui prouve que l'environnement des individus peut avoir un effet important sur la dynamique d'un modèle et ses résultats. 
=======
>>>>>>> 8867ed209b4948c34c5e57e433769fd751b85752
