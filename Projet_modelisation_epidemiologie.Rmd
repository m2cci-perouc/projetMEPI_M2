...
sol <- modAppli(Par)
colnames(sol) <- c("Tx_infecté", "Last_incidence", "Nbr_infectes", "YearIncidence")
sol

```

Avec les valeurs nominales, une vague d'infection se propage dans la population mais la maladie est rapidement ralentie car des individus sont remis. Néanmoins, l'agent pathogène reste présent dans la population avec une proportion d'infectés stable. \
La modélisation dure 2 ans donc les sorties calculées le dernier jour sont stables et renseignent sur la dynamique du modèle.

Le taux d'infectés au dernier jour de la modélisation  est de 0,18.\
L'incidence au dernier jour de la modélisation est de 0,65.\
Le nombre maximal d'infectés en une journée est de 37,2.\
La somme de l'incidence sur une année est de 280,9.
...
Nous faisons l'hypothèse que la transmission de l'agent pathogène est plus grande au sein d'une même classe qu'entre les classes d'âge.
